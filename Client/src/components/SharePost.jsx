...
function SharePost() {
  ...
  return (
    <form onSubmit={handlePost} className='sharepost flex flex-col w-2/3 mt-5 rounded-2xl items-center bg-second'>
        ...
        <label class="block  mb-2 text-sm font-medium text-gray-900 dark:text-white" for="file_input">Post Image</label>
        <input required onChange={(e)=>{setFile(e.target.files[0])}} multiple={false} accept="image/png, image/jpeg" class="block w-25 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="file_input" type="file"></input>
        <input type='submit' class="mt-2 mb-2 bg-first w-20 rounded hover:bg-fourth text-white hover:text-black"></input>
