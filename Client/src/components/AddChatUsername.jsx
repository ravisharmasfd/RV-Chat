...
import fetchPerson from '../controllers/fetchPerson.js';

function AddChatUsername({addChatChannel}) {
    const searchRef = useRef();
    async function addChat(){
      const userName = searchRef.current.value;
      const res = await fetchPerson(userName);
      if(res){
        addChatChannel(res._id);    
  ...
  return (
    ...
    <div className="flex">
      ...
      <input type="button" onClick={addChat} 
        value="Start chat" className="bg-third p-2 rounded-tr-lg rounded-br-lg text-white font-semibold hover:bg-first transition-colors"/>
    </div>
...
export default AddChatUsername